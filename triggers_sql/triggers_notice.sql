...
DELIMITER $$
CREATE TRIGGER `before_delete_wine` BEFORE DELETE ON `wine` FOR EACH ROW BEGIN
    IF  ((SELECT COUNT(*) FROM notice WHERE OLD.id_wine = notice.id_wine) > 0) 
 
      THEN
        DELETE FROM notice WHERE notice.id_wine = OLD.id_wine;
    END IF;
